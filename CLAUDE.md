...
- **Git: commit and push freely, in logical groups, on a feature branch.** No need to ask permission first — once a change is complete and verified (build/typecheck/tests as applicable), commit it and push. Default to **multiple logically-grouped commits** (not one mega-commit). Cite code by symbol name in commit messages (see [`docs/patterns.md`](./docs/patterns.md)). Commit messages and PR titles follow [Conventional Commits](https://www.conventionalcommits.org/) (`type(scope): subject`, e.g. `fix(trades): gate opportunities on killer distance too`) — CI (`.github/workflows/pr-checks.yml`) enforces both, plus a completed PR-template checklist. Opening a PR and merging one are separate, still-gated actions — see the merge-approval rule below.
...
- **Merging a PR requires its own live approval.** This is a hard rule — see AGENTS.md's "Merging a PR requires its own live approval." Opening a PR, pushing fixes to one you opened, and merging one are different levels of authorization; approval to merge one PR (including "merge once CI is green") never carries over to a follow-up, precursor, or fix PR, even a trivial one — ask again, naming that specific PR, every time. Don't open a PR the user hasn't asked for either, even as an obvious next step.
...
- **Canonical sort is Wins → RWR% → ADR** (all descending). Use `canonicalSort()` from `src/lib/util.ts` everywhere player rows are ranked. Never sort by ADR alone.
