...
describe('useTabState', () => {
  test('falls back to defaultTab when the param is missing', () => {
    const { result } = renderHook(() => useTabState(TABS, 'leaderboard'), { wrapper: UrlStateProvider });
  ...
  test('supports a custom param name for co-existing tab bars (e.g. `view` + `tab`)', () => {
    ...
    const { result } = renderHook(() => useTabState(['regular', 'gauntlet'] as const, 'regular', 'view'), { wrapper: UrlStateProvider });
