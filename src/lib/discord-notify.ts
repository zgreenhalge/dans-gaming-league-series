...
// score route's post-commit hooks via afterBestEffort()).
...
async function postEmbed(webhookUrl: string, embed: { title: string; description?: string; color: number }): Promise<void> {
  try {
    ...
    if (!res.ok) console.error(`[discord-notify] webhook returned ${res.status}`);
  } catch (e) {
    console.error('[discord-notify] webhook post failed:', e);
  }
}
...
export async function notifyMatchServerLive(matchId: number): Promise<void> {
...
export async function notifyMatchScoreReported(matchId: number): Promise<void> {
  const webhookUrl = process.env.DISCORD_MATCH_NOTIFICATIONS_WEBHOOK_URL;
  if (!webhookUrl) return; // Not configured — skip before doing any DB work.
