...
| `match_kills` | Demo-derived kill fact table, one row per kill (FK `match_id`; `attacker_player_match_stats_id`/`victim_player_match_stats_id`/`assister_player_match_stats_id` FK `player_match_stats`, attacker/assister nullable for a non-roster kill): `round_number`, `weapon` (individual weapon, not a category), `headshot`, `noscope`, `wallbang`, `blind_kill`, `is_teamkill`, `tick`. Unique on `(match_id, round_number, victim_player_match_stats_id)`. Kills-by-weapon, killed-by-weapon, category rollups, and favorite weapon are all computed at query time from these rows rather than pre-aggregated. Written only when a demo is parsed. See [`demo-ingestion.md`](./demo-ingestion.md). |
