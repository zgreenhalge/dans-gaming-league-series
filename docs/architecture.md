...
| `player_match_economy_stats` | Demo-derived shot/accuracy/damage/rounds breakdown, several rows per `player_match_stats` row (FK `player_match_stats_id`, unique with `economy_type`) — one per round-economy tier (`eco`/`half_buy`/`force_buy`/`full_buy`). Written only when a demo is parsed. See [`demo-ingestion.md`](./demo-ingestion.md). |
