...
| `discord_event_sync` | `season` (regular), or `match` | `syncSeasonScheduledEvents()` (`discord-event-sync.ts`, #398) — a real Discord API failure listing the guild's scheduled events (season-level), or a Supabase write failure updating a matched match's `scheduled_at` (match-level). A match with no matching scheduled event yet is not an error |
