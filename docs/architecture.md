...
| `player_match_weapon_stats` | Demo-derived shot/accuracy/damage/rounds breakdown, several rows per `player_match_stats` row (FK `player_match_stats_id`, unique with `weapon`) — one per exact weapon fired (e.g. `ak47`), gated to guns only. `weapon_category` (`pistol`/`smg`/`rifle`/`sniper`/`shotgun`) is derived from `weapon` at query time rather than trusted as stored — the column stays live and `not null` only so a row from a match not yet reparsed since `weapon` was added still resolves a category; #499 tracks dropping it once every match is confirmed reparsed. Written only when a demo is parsed. See [`demo-ingestion.md`](./demo-ingestion.md). |
