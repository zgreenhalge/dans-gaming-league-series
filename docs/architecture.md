...
| `discord_notify_reminder` | `match` | Covers every failure surface of the 1-hour-out reminder: `schedule_match_reminder()` (Postgres function) recording that the `cron_secret` Vault secret is missing, `PATCH /api/matches/[id]/schedule` recording that the `schedule_match_reminder` RPC call itself errored or threw, and `notifyMatchReminder()` (`discord-notify.ts`, #395) recording a real webhook failure — one admin-visible signal for "the reminder broke" regardless of which of the three failed |
