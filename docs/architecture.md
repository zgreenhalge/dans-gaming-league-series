...
| `player_match_sabremetrics` | Demo-derived advanced stats, one row per `player_match_stats` row (FK `player_match_stats_id`): `damage_ct`/`damage_t`, KAST, clutches, utility, objectives. Written only when a demo is parsed. `headshot_kills`/`teamkills`/`opening_kills`/`opening_deaths`/`two_k_rounds` aren't stored here — all duplicate `match_kills` data, so they're derived at query time (`deriveHeadshotAndTeamkillCounts()`/`deriveOpeningDuelCounts()`/`deriveTwoKRoundCounts()` in `queries/kills.ts`). `shots_fired`/`shots_hit`/`headshot_hits` also aren't stored here — they duplicate `player_match_weapon_stats`, so they're summed at query time (`deriveAccuracyTotals()` in `queries/weaponStats.ts`). `kills_ct`/`_t`, `deaths_ct`/`_t`, `assists_ct`/`_t`, and `headshot_kills_ct`/`_t` likewise aren't stored — they combine `match_kills` with each round's `match_rounds.shirts_side` and the player's `player_match_stats.faction`, resolved via `resolvePlayerSide()` and summed via `deriveSideSplitCounts()` (`queries/kills.ts`). `damage_ct`/`damage_t` stay stored — no fact table records per-round per-side damage the way `match_kills` does for kills. See [`demo-ingestion.md`](./demo-ingestion.md). |
