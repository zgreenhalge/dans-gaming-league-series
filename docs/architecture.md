...
| `player_match_economy_stats` | Demo-derived shot/accuracy/damage/rounds breakdown, several rows per `player_match_stats` row (FK `player_match_stats_id`, unique with `economy_type`) — one per round-economy tier (`save`/`eco`/`force`/`full_buy`). Written only when a demo is parsed. See [`demo-ingestion.md`](./demo-ingestion.md). |
...
| `match_damage_events` | Demo-derived damage fact table, one row per `player_hurt` event (FK `match_id`; `victim_player_match_stats_id` FK `player_match_stats`, not null; `attacker_player_match_stats_id` FK `player_match_stats`, nullable for a non-roster attacker such as world/fall damage): `round_number`, `weapon`, `damage`, `hitgroup`, `tick`. `damage` is health actually lost — clamped to what the victim had left that round, mirroring the engine's own `m_iDamage` accumulator, since a `player_hurt` event's raw `dmg_health` isn't itself capped at the victim's remaining health. Self-damage and teamdamage are kept, not filtered — same query-time-judgment convention as `match_utility_throws`. `weapon` includes grenade/utility damage (`hegrenade`, `molotov`/`inferno`) alongside guns, so this table needs no separate reconciliation against `match_utility_throws` to cover utility damage. Written only when a demo is parsed; read via `getMatchDamageEvents()` (`queries/damage.ts`) for the match H2H tab's per-pair damage bar (`computeMatchDuels()`, `queries/duels.ts`). See [`demo-ingestion.md`](./demo-ingestion.md). |
...
Wired into twenty-nine operations today:
...
| `discord_thread_create` | `match` | `publishWeekThreads()` (`discord-threads.ts`, #398) — either a real Discord API failure creating that match's thread, or an idempotency skip (a thread for this match already exists — found by a previously-recorded `match_discord_state.thread_id` if still live, else by exact title match against `listChannelThreads()`'s read of the channel's actual threads, so a thread an admin created by hand is adopted instead of duplicated), recorded the same "needs admin eyes" way as the gauntlet roster-drift case |
| `discord_thread_member_add` | `match` | `addThreadMembers()` (`discord-threads.ts`) — a real Discord API failure explicitly adding one of a newly created thread's rostered, Discord-linked participants as a thread member (mentioning them in the thread's own starter message pings but doesn't reliably add them). Best-effort per participant: one failure doesn't stop the rest, and none of them fail the thread creation itself |
...
(season/match name, or "EHOG Recompute" for `system`). The admin console's Activity feed
(`AdminActivityFeed.tsx`), filtered to the Errored tag, lists all of them, merged with failed background
jobs; Manage → Season
