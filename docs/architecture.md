...
| `match_discord_state` | One row per match (`match_id` PK, `ON DELETE CASCADE`). `thread_id` is the match's Discord forum-thread id, written by `publishWeekThreads()` (`src/lib/discord-threads.ts`, #398) once its thread is created — lets a re-publish tell "already has a thread" from "needs one." `reminder_sent_at` is reserved for a planned 1-hour-out match reminder (#395), not yet built. |
