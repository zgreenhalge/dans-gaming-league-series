...
| `player_match_sabremetrics` | Demo-derived advanced stats, one row per `player_match_stats` row (FK `player_match_stats_id`): CT/T side splits, opening duels, KAST, clutches, utility, objectives. Written only when a demo is parsed. `headshot_kills`/`teamkills` aren't stored here — both duplicate `match_kills` data, so they're derived at query time (`deriveHeadshotAndTeamkillCounts()` in `queries/kills.ts`). See [`demo-ingestion.md`](./demo-ingestion.md). |
