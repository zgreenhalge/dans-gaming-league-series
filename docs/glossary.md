...
- **Gauntlet** — a season format (`is_gauntlet = true`) that runs as a single-elimination bracket
  ...
  - **All gauntlet matches are stored with `is_playoff_game = true`**, so the regular
    ...
    `getGauntletRounds`)
  - **Pod scheduling** — a pod's two games share the same 4 players, so they're always played
    sequentially, never concurrently. Each game's `scheduled_at` is independently editable via
    `PATCH /api/matches/[id]/schedule`; a synced Discord Scheduled Event drives Game 1's thread and
    derives Game 2's time 30 minutes later automatically, but a human edit never pairs the two. See
...
| Discord `@Participants` role sync | `src/lib/discord-roles.ts`, hooked from `src/app/api/seasons/[id]/players/route.ts` (POST/DELETE) and `src/lib/season-lifecycle.ts` (`activateSeason()`/`checkSeasonCompletion()`/`checkGauntletCompletion()`) |
