...
| Discord scheduled-event time sync-back (earliest event-share link found in a match's own thread, no `match_discord_state` dependency) | `src/lib/discord-event-sync.ts` (`syncSeasonScheduledEvents()`, `findFirstSharedEventId()`), `scripts/discord-event-sync.ts`, `.github/workflows/discord-event-sync.yml` (cron) |
