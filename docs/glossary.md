...
| Discord scheduled-event time sync-back (earliest event-share link found in a match's own thread, checkpointed via `match_discord_state.event_id`/`message_checkpoint` so a thread is only ever fully re-read once) | `src/lib/discord-event-sync.ts` (`syncSeasonScheduledEvents()`, `scanThreadHistory()`, `scanThreadSince()`), `scripts/discord-event-sync.ts`, `.github/workflows/discord-event-sync.yml` (cron) |
