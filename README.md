...
| `DISCORD_BOT_TOKEN` | Discord bot token — grants/revokes `@Participants` and manages per-player name-color roles (`src/lib/discord-roles.ts`, #397), publishes weekly match threads and closes them once played (`src/lib/discord-threads.ts`, #398), scans each match thread's message history for a shared scheduled event and syncs its start time into `scheduled_at` (`src/lib/discord-event-sync.ts`, #398), and registers slash commands (`scripts/register-discord-commands.ts`); the interactions endpoint itself is verified by `DISCORD_PUBLIC_KEY`, not this token. Needs `Manage Roles`, `Create Public Threads`, `Send Messages in Threads`, `Manage Threads` (to archive/lock a played match's thread), and `Read Message History` (to scan a thread for a shared event), plus a channel-level permission overwrite on each season's `season-{N}` forum channel if it restricts access — with the bot's own role positioned at the top of the server's role hierarchy, above `@Participants` and above where name-color roles get created. Reading guild scheduled events needs no extra permission beyond guild membership |
| `DISCORD_GUILD_ID` | The DGLS Discord server's id — scopes the `@Participants`/name-color role, match-thread, and scheduled-event API calls to the right guild |
| `DISCORD_PARTICIPANTS_ROLE_ID` | The `@Participants` role's id — what `src/lib/discord-roles.ts` grants/revokes |
| `CRON_SECRET` | Bearer token checked by `GET /api/cron/refresh-steam` (Vercel cron) and `POST /api/cron/match-reminder` (Supabase pg_net-triggered). The same value must also be stored in Supabase Vault as the `cron_secret` secret — `schedule_match_reminder()` reads it from there to call the latter endpoint, since a Postgres function has no access to Vercel's env vars. Rotate in both places together; there's no automated sync between them |
