...
| `DISCORD_BOT_TOKEN` | Discord bot token — grants/revokes `@Participants` and manages per-player name-color roles (`src/lib/discord-roles.ts`, #397), publishes weekly match threads (`src/lib/discord-threads.ts`, #398), and registers slash commands (`scripts/register-discord-commands.ts`); the interactions endpoint itself is verified by `DISCORD_PUBLIC_KEY`, not this token. Needs `Manage Roles`, `Create Public Threads`, and `Send Messages in Threads`, plus a channel-level permission overwrite on each season's `season-{N}` forum channel if it restricts access — with the bot's own role positioned at the top of the server's role hierarchy, above `@Participants` and above where name-color roles get created |
